...
class AnomalyMapGenerator(nn.Module):
    ...
    def __init__(self, image_size: Union[ListConfig, Tuple], sigma: int = 4):
        super().__init__()
        self.image_size = image_size if isinstance(image_size, tuple) else tuple(image_size) # if image_size is tuple: assign image_size, else: 先把image_size變成tuple再siign給等號左邊
    ...
    def compute_distance(embedding: Tensor, stats: List[Tensor]) -> Tensor:
        ...
        batch, channel, height, width = embedding.shape # shape.[3 960 56 56]
        embedding = embedding.reshape(batch, channel, height * width) #torch.Size([3, 960, 3136])

        # calculate mahalanobis distances
        mean, inv_covariance = stats #mean.Size([960, 3136]), in_covariance.Size([3136, 960, 960])
        delta = (embedding - mean).permute(2, 0, 1)  # torch.Size([3136, 3, 960])

        distances = (torch.matmul(delta, inv_covariance) * delta).sum(2).permute(1, 0)
        distances = distances.reshape(batch, 1, height, width)
        distances = distances.clamp(0).sqrt() # clamp(0) 使小於0的數字都變成0，這樣後面接sqrt才不會出錯
    ...
    def forward(self, **kwargs):
        ...
        ---output = self.anomaly_map_generator(
                embedding=embeddings, mean=self.gaussian.mean, inv_covariance=MultiVariateGaussian(n_features, n_patches).inv_covariance
